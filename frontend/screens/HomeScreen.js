...
//   useEffect(() => {
//     fetch('http://192.168.0.2:3002/get-profile')
//       .then((res) => res.json())
//       .then((data) => {
//         if (data.name) {
...
const HomeScreen = ({ navigation }) => {
  ...
  const features = [
    ...
    },
    {
      ...
      title: 'Progress Tracker',
      description: 'Track learning goals with real-time analytics.',
      targetScreen: { tab: 'Quiz', screen: 'Results' },
    },
    {
      image: require('../assets/ach-pic.jpg'),
      title: 'Achievements',
  ...
  return (
    <ImageBackground
      source={require('../assets/bgg4.jpeg')}
    ...
    >
      ...
      <SafeAreaView style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <Text style={styles.helloText}>Welcome, {userName} 👋!!</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingTop: 20, marginBottom: 20 },
  helloText: { fontSize: 24, fontWeight: 'bold', color: '#fff' },
  growCard: {
    backgroundColor: '#3B82F6',
    borderColor:'snow',
    borderWidth:5,
  ...
  seeAllText: { fontSize: 16, color: 'snow ', fontWeight: '600' },
  subjectListContainer: { paddingHorizontal: 15, marginBottom: 30 },
  subjectCard: {
    backgroundColor: '#FFFFFF',
    borderRadius: 15,
