...
export default function SignupScreen({ navigation }) {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.0.3:3000/send-otp', { email });
      if (response.data.success) {
        Alert.alert('Success', `OTP sent to ${email}`);
        setOtpSent(true);
        setResendTimer(30);
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.0.3:3000/verify-otp', {
        email,
        otp: enteredOtp,
      });
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.3:3002/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name,
